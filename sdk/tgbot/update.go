...
func (u *Update) GetButton() Button {
	if u.btn == nil {
		...
		if err != nil {
			lgr.Printf("[ERROR] cannot find button %s, %v", u.CallbackData(), err)
		}
		u.btn = &button
	}
	return *u.btn
...
func (u *Update) GetChatInfo() *ChatInfo {
	if u.chat == nil {
		...
		if err != nil {
			lgr.Printf("[WARN] cannot find chat info, %v", err)
		}
		u.chat = &chat
	}
...
func (u *Update) GetChatInfoByActiveChainAndStep(chain string, step string) (ChatInfo, error) {
	return u.chatProv.GetChatInfoByActiveChainAndStep(chain, step)
}
