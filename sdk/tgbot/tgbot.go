...
type ChatProvider interface {
	...
	GetChatInfoByActiveChainAndStep(chain string, step string) (ChatInfo, error)
}
...
func (b *Bot) SaveUser(update *tgbotapi.Update) (User, error) {
	...
	user := User{UserId: &tgUser.ID, UserName: tgUser.UserName, DisplayName: tgUser.FirstName, LastName: tgUser.LastName, Status: UnKnow}
	user, err = b.chatProv.UpsertUser(user)
	if err != nil {
		return User{}, err
	}
	return user, nil
