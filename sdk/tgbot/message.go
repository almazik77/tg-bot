...
func (b *MessageBuilder) AddWebAppInfoButton(url string, text string) *MessageBuilder {
	button := tgbotapi.KeyboardButton{WebApp: &tgbotapi.WebAppInfo{URL: url}, Text: text}
	b.replyKeyboard[len(b.replyKeyboard)-1] = append(b.replyKeyboard[len(b.replyKeyboard)-1],
		button)
	return b

}

func (b *MessageBuilder) AddButtonSwitchForCurrentChat(text, sw string) *MessageBuilder {
	buttonSwitch := tgbotapi.NewInlineKeyboardButtonSwitch(text, sw)
	buttonSwitch.SwitchInlineQuery = nil
	buttonSwitch.SwitchInlineQueryCurrentChat = &sw
	b.keyboard[len(b.keyboard)-1] = append(b.keyboard[len(b.keyboard)-1],
		buttonSwitch,
	)
	return b
